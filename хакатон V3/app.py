...
def index():
    ...
    all_subs = (db.session.query(
                ...
                .filter(User.is_admin == False, User.is_verified == True)
                .group_by(db.func.date(Submission.created_at))
                .order_by('date')
                .all())
...
def register():
    ...
    if form.validate_on_submit():
        ...
        message = f"Код подтверждения Птичка: {code}"
...
def profile():
    ...
    # Определение места в рейтинге
    user_rank = None
    if current_user.is_verified and not current_user.is_admin:
        users = User.query.filter_by(is_verified=True, is_admin=False).all()
        rating = [(u.id, u.get_total_points()) for u in users]
        rating.sort(key=lambda x: x[1], reverse=True)
        for idx, (uid, pts) in enumerate(rating, 1):
...
def add_submission():
    ...
    if form.validate_on_submit():
        ...
        if not user or not material:
            flash('Ошибка: пользователь или материал не найден.', 'danger')
        ...
        db.session.commit()
        logger.info(f"Admin {current_user.full_name} added submission for {user.full_name}: {weight}kg of {material.name}, points: {points}")
